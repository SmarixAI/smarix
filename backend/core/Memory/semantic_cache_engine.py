...
class CacheMatch:
    ...
    # NEW: mark if this cached response is a "failure" (no context / error)
...
class EntityExtractor:
    ...
    def extract_entities(query: str) -> Dict[str, Set[str]]:
        ...
        # IMPORTANT: do NOT treat bare "#123" as PR unless explicitly tied to PR/pull.
        pr_patterns = [
            r"\bPR[#\s-]?(\d+)",
            r"\bpr[#\s-]?(\d+)",
            r"\bpull request[#\s-]?(\d+)",
        ...
        # Issue numbers: issue #123, issue-123, bug-123
        ...
        var_patterns = [r"`([a-z_][a-z0-9_]+)`", r'variable\s+["\']?(\w+)["\']?']
    ...
    def entities_match(
        ...
        # Critical entities must match exactly if present
        ...
        if important_mismatch:
            # Important mismatch: strong penalty
            return False, 0.5

        if has_critical_entities or has_important_entities:
            return True, 1.0

        # No entities at all: treat as generic query
        return True, 1.0
...
class UniversalConfidenceScorer:
    ...
    def calculate_freshness_score(
        age_hours: float, query_volatility: str = "medium"
    ) -> float:
    ...
    def calculate_overall_confidence(
        ...
        # weight in quality_score lightly to downrank low-quality answers
        quality_factor = 0.5 + 0.5 * max(0.0, min(1.0, quality_score))
        overall = adjusted_similarity * quality_factor * (0.90 + 0.10 * freshness_score)
...
class UniversalSemanticCache:
    ...
    def __init__(
        ...
        # NEW: do not reuse failure responses from cache
    ...
    # NEW: detect "failure" responses that should not be reused semantically
    @staticmethod
    def _is_failure_response(response: Dict[str, Any]) -> bool:
        """
        Treat 'no context', empty content, or explicit error flags as failures.
        This ensures they are not reused across semantically similar queries.
        """
        if not response:
            return True
    ...
    def get(self, query: str, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        # EXACT MATCH: okay to return even for failures, because user literally repeated
        exact_match = self._check_exact_match(query_hash, session_id)
        if exact_match:
            elapsed_ms = (time.time() - start_time) * 1000
            self.stats["exact_matches"] += 1
            # only count cost saving if not a failure
            if not self._is_failure_response(exact_match):
        ...
        try:
            ...
            if not best_match:
                ...
                logger.info(
                    f"❌ NO MATCH | query='{query[:60]}...' | time={elapsed_ms:.1f}ms"
                )
                return None

            # If best match is a failure (no-context/error) and we do not want to reuse failures,
            # treat as a miss and fall back to LLM.
            if best_match.is_failure and not self.cache_failures:
                elapsed_ms = (time.time() - start_time) * 1000
                self.stats["no_matches"] += 1
                logger.info(
                    f"🚫 SKIP FAILURE CACHE | query='{query[:60]}...' | "
                    f"matched='{best_match.cached_query[:60]}...' | time={elapsed_ms:.1f}ms"
                )
                return None
    ...
    def _find_best_match(
        ...
        for cache_key, cache_data in self.cache_index.items():
            ...
            overall_conf, breakdown = (
                UniversalConfidenceScorer.calculate_overall_confidence(
                    query1=query,
                    query2=cached_query,
            ...
            if overall_conf > best_confidence:
                ...
                is_failure = self._is_failure_response(cache_data["response"])
    ...
    def _decide_strategy(self, match: CacheMatch) -> CacheStrategy:
        ...
        # Never try to augment/context-hint with a failure response
        if match.is_failure:
        ...
        if conf_level == ConfidenceLevel.VERY_HIGH:
            if age <= self.fresh_hours:
                ...
                return (
                    CacheStrategy.AUGMENT
                    if self.enable_augmentation
                    else CacheStrategy.USE_AS_CONTEXT
                )
            else:
                return CacheStrategy.IGNORE
    ...
    def set(
        ...
        try:
            ...
            # Respect failure caching policy:
            # - Always store in Redis for exact repeat usability.
            # - Only index for semantic matching if allowed.
            is_failure = self._is_failure_response(response)

            redis_key = self._redis_key(session_id, query_hash)
            self.redis.set(redis_key, json.dumps(response), ttl=self.ttl)

            # Skip semantic indexing for failures when cache_failures=False
            if is_failure and not self.cache_failures:
                logger.info(
                    f"⚠️ NOT INDEXING FAILURE RESPONSE | query='{query[:60]}...'"
                )
            else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if total > 0:
            ...
            exact_rate = very_high_rate = high_rate = medium_rate = low_rate = (
                miss_rate
            ) = rejection_rate = overall_hit_rate = 0.0
        ...
        return {
            ...
            "confidence_levels": {
                ...
                "very_high": {
                    "count": self.stats["very_high_matches"],
                    "rate": round(very_high_rate, 2),
                },
                "high": {
