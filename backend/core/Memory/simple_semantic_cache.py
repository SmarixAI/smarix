...
class SimpleSemanticCache:
    ...
    def __init__(
        ...
        logger.info(
            f"Thresholds: Exact >={exact_threshold * 100:.0f}%, High >={high_threshold * 100:.0f}%, Medium >={medium_threshold * 100:.0f}%, Low >={low_threshold * 100:.0f}%")
    ...
    def get(self, query: str, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for cache_key, cache_data in self.cache_index.items():
                ...
                # If critical entities don't match, skip
                ...
                # Apply entity penalty to similarity
            ...
            if not best_match or best_similarity < self.low_threshold:
                elapsed_ms = (time.time() - start_time) * 1000
                self.stats['no_matches'] += 1
                logger.info(
                    f"NO MATCH | query='{query[:60]}...' | best_sim={best_similarity:.3f} | time={elapsed_ms:.1f}ms")
                return None

            age_hours = (time.time() - best_cache_data['timestamp']) / 3600
            freshness_factor = np.exp(-age_hours / 24.0)
            confidence = best_similarity * (0.85 + 0.15 * freshness_factor)
    ...
    def set(
        ...
        try:
            ...
            redis_key = f"simple:cache:{session_id}:{query_hash}"
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if total > 0:
            ...
            exact_rate = high_rate = medium_rate = low_rate = miss_rate = rejection_rate = overall_hit_rate = 0.0
