...
class ResponseHandler:
    ...
    def _has_executable_repo_code(self, chunks: List[Dict[str, Any]]) -> bool:
        """
        Returns True if we have at least one executable code chunk
        (not PR / issue / diff metadata).
        """
        for c in chunks:
            meta = self._get_metadata(c)
            chunk_type = meta.get("type") or meta.get("chunk_type")

            # Only real code chunks count
            if chunk_type == "code":
                content = c.get("content", "")
                if content and len(content.splitlines()) >= 10:
                    return True
    ...
    def respond_with_results(
        ...
        if query_type == QueryType.PR_ISSUE_CODING_QUESTION:
            ...
            if not has_code:
                self.chatbot.logger.warning(
                    "EARLY_PR_CODING_GUARD | No executable repo code found — downgrading query"
                )

                return self.package_response(
                    "This pull request does not contain enough executable repository code to generate a step-by-step coding tutorial. Here's a high-level explanation instead.",
                    github_results,
                    [],
