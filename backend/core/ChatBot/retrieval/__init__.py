...
class RetrievalMixin(
    ...
    def retrieve_github_first(
        ...
        # -------------------------------------------------------
        # FILE_QUERY: Try code_chunks_loader first (exact filename + path), then vector search if not found
        if query_type == QueryType.FILE_QUERY and query_text:
            self.logger.info(f"FILE_QUERY | Processing file query: {query_text}")
            ...
            if code_loader:
                ...
                if file_paths:
                    self.logger.info(f"FILE_QUERY | Searching code_chunks_loader for {len(file_paths)} file paths: {file_paths}")
                    for file_path in file_paths:
                        ...
                        chunks = code_loader.get_chunks_by_file_path(
                            ...
                            repo_owner,
                            repo_name,
                            exact_match_only=True  # Exact match only for file queries
                        )
                        ...
                        if not chunks and filename_hints:
                            ...
                            if filename:
                                self.logger.info(
                                    f"FILE_QUERY | No results for path '{file_path}', "
                        ...
                        self.logger.info(
                            f"FILE_QUERY | File path '{file_path}': found {len(chunks)} chunks from code_chunks_loader"
                        )
                ...
                if not results and filename_hints:
                    self.logger.info(
                        f"FILE_QUERY | No results from exact paths, trying filename hints: {filename_hints}"
                    )
                    for filename_hint in filename_hints:
                        ...
                        self.logger.info(
                            f"FILE_QUERY | Filename hint '{filename_hint}': found {len(chunks)} chunks"
                        )
                ...
                if results:
                    self.logger.info(
                        f"FILE_QUERY | Found {len(results)} chunks from code_chunks_loader, "
                    ...
                    self.logger.info(
                        f"FILE_QUERY | No results from code_chunks_loader, falling back to vector search"
                    )
