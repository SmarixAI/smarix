...
# ---- FILE LOOKUP (STRICT EVIDENCE BASED) ----
file_evidence_patterns = [
...
Feel free to ask me anything about the codebase!"""
    ...
    def llm_classify_query(self, query: str) -> str:
        ...
        8. FILE_LOOKUP - Questions asking where file is located, finding files, code in files
    ...
    def classify_query(self, query: str) -> str:
        ...
        has_file_evidence = any(re.search(p, query_lower) for p in file_evidence_patterns)
        mentions_file_word = bool(
            re.search(r'\bfile\b', query_lower) and
            any(k in query_lower for k in ['where', 'which', 'show', 'find', 'open', 'locate'])
        )

        # Class / Type name detection (PascalCase identifiers)
        class_name_pattern = (
            r'\b[A-Z][a-zA-Z0-9]+'
            r'(Controller|Service|Manager|Repository|Bloc|ViewModel)\b'
        )
        
        SNAKE_CASE_FILE_PATTERN = r'\b[a-z][a-z0-9_]{2,}\b'
        mentions_class_name = bool(re.search(class_name_pattern, query))

        mentions_snake_case_file = bool(
            'file' in query_lower and
            re.search(SNAKE_CASE_FILE_PATTERN, query_lower)
        )

        if has_file_evidence or mentions_file_word or mentions_class_name or mentions_snake_case_file:
            self.logger.info("CLASSIFICATION | Rule-based: FILE_LOOKUP (explicit file reference)")
            return QueryType.FILE_LOOKUP



        # 2️⃣ RANDOM / GENERATIVE FLOWS
        ...
        # 4️⃣ PR / ISSUE / COMMIT (specific entities)
        ...
        # 7️⃣ CONCEPTUAL (LOWEST PRIORITY RULE)
