...
async def startup():
    ...
    if not github_db_path:
        ...
        print("    Required: data/VectorDB/multi_index directory with index files")
...
def run_vectordb_build() -> Dict[str, Any]:
    ...
    try:
        ...
        if not vectordb_root.exists():
            return {"success": False, "error": "VectorDB directory not found"}
        ...
        return {
            "success": True,
            "indices": indices,
            "message": f"Successfully built {len(indices)} vector indices"
        }
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def run_onboarding(generators_to_run: list = None) -> Dict[str, Any]:
    ...
    try:
        ...
        vectordb_path = None
        possible_paths = [
            repo_root / "data" / "VectorDB" / "multi_index",
            Path(__file__).resolve().parent.parent.parent / "data" / "VectorDB" / "multi_index",
            Path("../../data/VectorDB/multi_index").resolve(),
            Path("data/VectorDB/multi_index").resolve(),
        ]
        
        for path in possible_paths:
            if path.exists() and (path / "code" / "faiss.index").exists():
                vectordb_path = str(path)
                break
        
        if not vectordb_path:
            return {"success": False, "error": "Vector database not found. Please run data pipeline first."}
