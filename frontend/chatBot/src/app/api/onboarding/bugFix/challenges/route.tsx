  ...
  try {
    ...
    try {
      ...
      if (s3Error.name === 'NoSuchKey') {
        return NextResponse.json({ error: 'Not found', key: s3Key }, { status: 404 });
      }
      throw s3Error;
    }
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    let existingData: any = { 
      metadata: { total_submissions: 0, last_updated: new Date().toISOString() }, 
      submissions: [] 
    };

    try {
      const getCommand = new GetObjectCommand({ Bucket: S3_BUCKET, Key: s3Key });
      const s3Response = await s3Client.send(getCommand);
      if (s3Response.Body) {
        existingData = JSON.parse(await s3Response.Body.transformToString());
      }
    } catch (e: any) { /* Ignore if new file */ }
