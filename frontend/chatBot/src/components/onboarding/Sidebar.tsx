...
interface SidebarProps {
  ...
  totalModules: number;
  activeTab: string;             
  qnaSections?: any[];           // Changed to any[] to include module data with progress
  onSelectQnASection?: (key: string) => void; 
  practiceTasks?: any[];
...
export default function Sidebar({
  ...
  // ------------------------------------------------------------------
  //     🟣 IF ACTIVE TAB IS Q&A → SHOW MINDMAP STYLE SIDEBAR
  // ------------------------------------------------------------------
  if (activeTab === "qa") {
    return (
      <aside className="col-span-3 space-y-4">
        ...
        {/* ---------------------- Q&A MINDMAP ---------------------- */}
        <div
          className={`rounded-2xl p-4 transition-colors hover:bg-opacity-90 shadow-xl relative overflow-hidden ${
        ...
        >
          ...
          <h3 className="font-bold mb-1.5 flex items-center space-x-2 relative z-10 text-base">
            ...
            <span>Q&A Mindmap</span>
          </h3>
          <p className={`text-xs mb-3 ${darkMode ? "text-gray-400" : "text-slate-600"}`}>
            Explore all topics & question sets
          </p>

          <nav className="space-y-3 mt-4">
            {qnaSections && qnaSections.length > 0 ? (
              qnaSections.map((section) => {
                const sectionId = section.id || section.moduleId;
                ...
                const score = section.score !== undefined ? section.score : null;
                const totalQuestions = section.totalQuestions || 0;

                return (
                  <button
                    key={sectionId}
                    onClick={() => onSelectQnASection?.(sectionId)}
                    className={`w-full flex flex-col px-3 py-3 rounded-lg text-sm transition-colors group ${
                  ...
                  >
                    <div className="flex items-center justify-between w-full mb-1">
                      ...
                      <ChevronRight className="w-4 h-4 opacity-60 group-hover:translate-x-1 transition-transform flex-shrink-0" />
                    </div>
                    ...
                    <div className={`w-full h-1.5 rounded-full mt-2 ${darkMode ? "bg-gray-700" : "bg-slate-200"}`}>
                      <div
        ...
        {/* ---------------------- QA PROGRESS CARD ---------------------- */}
        <div
          className={`rounded-2xl p-4 shadow-xl transition-all hover:scale-105 ${
            darkMode ? "glass-card-dark" : "glass-card-light"
          }`}
        >
          <h3 className="font-bold flex items-center space-x-2 text-base">
            <span>📊</span>
          ...
          <div className="flex items-center justify-between mt-2 mb-1">
            ...
            >
              {qnaSections?.filter((s: any) => s.status === 'completed').length || 0}/{qnaSections?.length || 0}
            </span>
          </div>

          {/* Average Score Display */}
          {qnaSections && qnaSections.length > 0 && (() => {
            const sectionsWithScores = qnaSections.filter((s: any) => 
              s.score !== null && s.score !== undefined && s.totalQuestions && s.totalQuestions > 0
            );
            if (sectionsWithScores.length > 0) {
              const totalScore = sectionsWithScores.reduce((sum: number, s: any) => sum + (s.score || 0), 0);
              const totalQuestions = sectionsWithScores.reduce((sum: number, s: any) => sum + (s.totalQuestions || 0), 0);
              const avgScore = totalQuestions > 0 ? Math.round((totalScore / totalQuestions) * 100) : 0;
              const avgScoreDisplay = totalQuestions > 0 ? `${totalScore}/${totalQuestions}` : 'N/A';
              
              return (
                <div className="mt-2 pt-2 border-t border-gray-700/50 dark:border-slate-200/20">
                  <p className={`text-xs ${darkMode ? "text-gray-400" : "text-slate-600"} mb-1`}>
                    Average Score
                  </p>
          ...
          <div
            className={`h-2.5 rounded-full overflow-hidden shadow-inner mt-2 ${
          ...
          >
            <div
              ...
              style={{ 
                width: `${qnaSections && qnaSections.length > 0 
                  ? Math.round((qnaSections.filter((s: any) => s.status === 'completed').length / qnaSections.length) * 100)
                  : 0}%` 
              }}
            ></div>
          </div>
        ...
        {/* ---------------------- QUICK TIP CARD ---------------------- */}
        <div
          className={`rounded-2xl p-4 shadow-xl transition-all hover:scale-105 relative overflow-hidden ${
            darkMode ? "glass-card-dark" : "glass-card-light"
          }`}
        >
          ...
          >
            ...
            Don't hesitate to ask questions to chatbot!
          </p>
