...
export default function OverviewModal({
  ...
  // ✅ NEW: State to track if repo is missing to show specific UI
        ...
        try {
          await fetch("/api/onboarding/progress", {
            ...
            body: JSON.stringify({
              ...
              repo: repo, // ✅ Pass repo to backend
            }),
          });
        } catch (error) {
  ...
  }, [isOpen, employeeId, moduleId, activeRepos]); // Added activeRepos dependency

  useEffect(() => {
    if (isOpen && moduleId) {
      setIsLoading(true);
      setMissingRepo(false); // Reset error state
      ...
      const fetchModuleContent = async () => {
        try {
          ...
          // ✅ Guard Clause: If no repo, stop and show error UI
          if (!repo) {
            console.warn(
  ...
  }, [isOpen, moduleId, activeRepos]); // Don't forget activeRepos in dependency
  ...
  const stats = useMemo(() => {
    ...
    const estimatedReadTime = Math.ceil(totalItems * 1.5);
  ...
  return (
    <div className="fixed inset-0 z-[200]" onClick={handleBackdropClick}>
      ...
      <div className="absolute inset-0 modal-backdrop bg-gradient-to-br from-[#0E1B2E]/70 via-[#0E1B2E]/60 to-blue-900/60 pointer-events-none" />
      ...
      >
        ...
        <div className="flex-1 grid grid-cols-[260px_1fr_320px] min-h-0">
          ...
          <aside className="h-full overflow-y-auto border-r border-slate-200 bg-white/70 backdrop-blur-xl p-4">
            <h3
              className={`${inter.className} text-xs font-bold text-slate-500 mb-4`}
            >
              Sections
            </h3>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-32 text-center animate-fade-in">
                ...
                <p className="text-sm text-slate-500 mt-2 max-w-xs leading-relaxed">
                  Please contact your manager to assign an active repository to
                  your account.
                </p>
              </div>
            ) : !activeItem ? (
            ...
            <div className="rounded-2xl p-4 bg-white shadow-sm border">
              ...
              <div className="w-full h-2 rounded-full bg-slate-200 overflow-hidden mt-2">
                <div
                  ...
                  style={{
                    width: `${sections.length > 0 ? Math.round(((currentSectionIndex + 1) / sections.length) * 100) : 0}%`,
                  }}
                />
              </div>
              ...
              {flatItems.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center transition-all ${isActive ? "scale-110" : "opacity-60 hover:opacity-100"}`}
                  >
                    {item.type === "qna" ? (
                      <ClipboardCheck
                      ...
                      <span
                        className={`h-2 rounded-full transition-all ${isActive ? "bg-gradient-to-r from-blue-500 to-indigo-500 w-10" : "bg-slate-300 w-2"}`}
                      />
                    )}
                  </button>
                );
