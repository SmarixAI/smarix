...
import { 
  CheckCircle, ChevronDown, ChevronUp, FileText, Brain, 
  HelpCircle, Upload, Loader2, ShieldCheck, 
  AlertCircle, FileCheck, Info, Target, RefreshCcw, ArrowUpCircle
} from 'lucide-react';
...
export default function EmployeeDocumentationSection({ employeeId, darkMode = false }: Props) {
  ...
  const [taskAIStates, setTaskAIStates] = useState<Record<string, TaskAIStatus>>({});
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const employee = data.employees?.find((e: any) => String(e.employeeId) === String(employeeId)) ?? data.employees?.[0];
  ...
  const handleFileUpload = (taskId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTaskAIStates(prev => ({
      ...prev,
      [taskId]: { ...prev[taskId], status: 'analyzing', progress: 0, fileName: file.name }
    }));

    let prog = 0;
    const interval = setInterval(() => {
      prog += 10;
      setTaskAIStates(prev => ({ ...prev, [taskId]: { ...prev[taskId], progress: prog } }));
      if (prog >= 100) {
        clearInterval(interval);
        setTimeout(() => {
          setTaskAIStates(prev => ({ ...prev, [taskId]: { ...prev[taskId], status: 'completed', score: 85 } }));
        }, 500);
      }
    }, 60);
    e.target.value = '';
  ...
  if (loading) return <Loader darkMode={false} message="Analyzing documentation requirements..." size="md" />;
    ...
    <div className="w-full h-full flex flex-col pt-6 pb-0">
      ...
      <div className="flex-1 w-full rounded-[2rem] border-2 border-slate-200 shadow-lg bg-white/70 backdrop-blur-sm flex flex-col min-h-0 overflow-hidden">
        ...
        <div className="px-8 py-6 border-b-2 border-slate-200 bg-slate-50/50 flex justify-between items-center">
          <div>
            <h2 className={`${inter.className} text-2xl font-extrabold text-[#0E1B2E] tracking-tight`}>Handover Documentation</h2>
            <p className="text-sm text-slate-500 font-medium">Systematic verification of knowledge transfer assets</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y-2 divide-slate-200">
          {filteredTasks.map(task => {
            ...
            const ai = taskAIStates[task.id] || { status: 'idle', progress: 0, score: null };

            return (
              <div key={task.id} className={`transition-all duration-300 ${isDone ? 'bg-emerald-50/20' : isExpanded ? 'bg-slate-50/50' : ''}`}>
                <div className="px-8 py-5">
                  <div className="flex justify-between items-center gap-6">
                    <div className="flex-1 cursor-pointer flex items-center gap-4" onClick={() => toggleTaskDetails(task.id)}>
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center border-2 ${isDone ? 'bg-emerald-100 border-emerald-200 text-emerald-600' : 'bg-white border-slate-200 text-slate-400 shadow-sm'}`}>
                        {isDone ? <CheckCircle className="w-6 h-6" /> : <FileText className="w-5 h-5" />}
                      </div>
                      <div>
                        <h4 className={`${inter.className} text-base font-bold text-[#0E1B2E] flex items-center gap-3`}>
                          {task.title || task.id}
                          <span className={`px-2 py-0.5 rounded text-[9px] font-black border uppercase tracking-tighter ${getPriorityStyles(task.priority)}`}>
                            {task.priority}
                          </span>
                        </h4>
                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wide mt-0.5">ID: {task.id}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {isDone && (
                        <span className={`${inter.className} flex items-center gap-1.5 text-xs font-bold text-green-700 bg-green-50 px-3 py-2 rounded-xl border-2 border-green-200 animate-in fade-in zoom-in-95`}>
                      ...
                      {ai.status === 'analyzing' && (
                        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border border-blue-100 shadow-sm">
                          ...
                          <span className={`${jetbrainsMono.className} text-[10px] font-bold text-blue-600`}>{ai.progress}% Audit</span>
                        </div>
                      )}
                      ...
                      {ai.status === 'completed' && !isDone && (
                        <div className="flex items-center gap-3 animate-in zoom-in-95 duration-300">
                          <div className="bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-200 flex items-center gap-2">
                             ...
                             <span className="text-xs font-black text-emerald-700">{ai.score}% Compliance</span>
                          </div>
                         <button 
  onClick={() => setCompletedTaskIds(prev => new Set(prev).add(task.id))}
...
>
  ...
  <span>Mark Complete</span>
                      ...
                      <input type="file" ref={el => { fileInputRefs.current[task.id] = el; }} onChange={(e) => handleFileUpload(task.id, e)} className="hidden" />
                      <button onClick={() => toggleTaskDetails(task.id)} className="p-2 text-slate-300 hover:text-slate-600 transition-colors">
                    ...
                    <div className="mt-6 mb-2 animate-in fade-in slide-in-from-top-4 duration-300">
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 rounded-2xl bg-white border-2 border-slate-200 shadow-lg overflow-hidden">
                        ...
                        <div className="lg:col-span-7 p-8 space-y-8 border-r-2 border-slate-200">
                          <section>
                            <div className="flex items-center gap-2 mb-3 text-slate-400">
                              ...
                              <h5 className="text-[10px] font-black uppercase tracking-widest">Requirement Overview</h5>
                            </div>
                          ...
                          <section className="bg-slate-50/50 p-6 rounded-xl border border-slate-200">
                            <div className="flex items-center gap-2 mb-4 text-blue-600">
                              ...
                              <h5 className="text-[10px] font-bold uppercase tracking-widest">Points to Cover</h5>
                            </div>
                            <ul className="space-y-3">
                              {task.questions?.map((q, i) => (
                                <li key={i} className="text-sm text-[#0E1B2E] font-bold flex items-start gap-3">
                        ...
                        <div className="lg:col-span-5 p-8 bg-[#FBFDFF] flex flex-col justify-center">
                          <div className={`h-full rounded-2xl p-6 border flex flex-col transition-all duration-500 ${ai.status === 'completed' ? 'bg-white border-emerald-200 shadow-md' : 'bg-slate-50/50 border-dashed border-slate-300'}`}>
                            {ai.status === 'completed' ? (
                              <div className="space-y-5">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <Brain className="w-5 h-5 text-indigo-600" />
                                    <span className="text-[10px] font-black uppercase text-slate-800 tracking-tight">AI Compliance Report</span>
                                  </div>
                                  <span className="text-2xl font-black text-emerald-600 tabular-nums">{ai.score}%</span>
                                </div>
                                
                                <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100 flex gap-3 items-center">
                                  <FileCheck className="w-4 h-4 text-emerald-600 flex-shrink-0" />
                                  <p className="text-[11px] font-bold text-emerald-800 truncate">{ai.fileName}</p>
                                </div>

                                <div className="space-y-2.5 py-2">
                                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                     <ArrowUpCircle className="w-3 h-3" /> Potential Improvements
                                  </p>
  ...
  {[
    ...
    >
      ...
      {item}
                                ...
                                >
                                  <RefreshCcw className="w-3 h-3" /> Re-upload & Re-Audit
                                </button>
                              </div>
                            ) : (
                              <div className="text-center space-y-3 py-6">
                                ...
                                <div>
                                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Awaiting Analysis</p>
                                  <p className="text-[10px] text-slate-300 mt-1 max-w-[180px] mx-auto font-medium">Neural verification will initiate upon file upload.</p>
                                </div>
                              </div>
                            )}
                          </div>
