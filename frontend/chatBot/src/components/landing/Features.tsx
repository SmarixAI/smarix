...
import { Zap, Shield, Code2, Terminal, Cpu, Layout, GitBranch, Share2, Search, Bug } from 'lucide-react';
...
const features = [
  {
    ...
    icon: Zap,
    color: '#3B82F6', 
    bgGradient: 'radial-gradient(circle at 0% 0%, rgba(59,130,246,0.15) 0%, transparent 50%)',
    points: [
      { icon: Cpu, title: 'Tech Stack Intel', desc: "AI explains architecture & design patterns tailored to you." },
      { icon: Terminal, title: 'Guided Practice', desc: "Explore real code and see how changes propagate safely." },
      { icon: Bug, title: 'Real-World Bugs', desc: "Simulate historical bugs to learn root cause analysis." }
    ]
  },
  {
    id: 1,
    ...
    icon: Shield,
    color: '#6366F1', 
    bgGradient: 'radial-gradient(circle at 100% 50%, rgba(99,102,241,0.15) 0%, transparent 50%)',
    points: [
      { icon: Layout, title: 'Docs Reflect Code', desc: "AI identifies and fixes gaps in documentation automatically." },
      { icon: Share2, title: 'Structured Handover', desc: "Capture status, decisions, and risks for new owners." },
      { icon: Shield, title: 'Tribal Knowledge', desc: "Extract undocumented insights via AI exit interviews." }
    ]
  },
  {
    id: 2,
    ...
    icon: Code2,
    color: '#10B981', 
    bgGradient: 'radial-gradient(circle at 50% 100%, rgba(16,185,129,0.15) 0%, transparent 50%)',
    points: [
      { icon: Search, title: 'Contextual Intel', desc: "Precise answers sourced from your actual code and docs." },
      { icon: Terminal, title: 'Smart Debugging', desc: "Analyze stack traces against system architecture instantly." },
      { icon: GitBranch, title: 'IDE Integration', desc: "Get guidance and best practices without breaking flow." }
    ]
  },
];
...
export const Features = () => {
  ...
  useEffect(() => {
    const unsubscribe = smoothProgress.onChange((latest) => {
      ...
      const newIndex = Math.min(Math.floor(adjustedProgress * 3), 2);
  ...
  return (
    <section ref={containerRef} className="relative h-[300vh] bg-white">
      <div id="onboarding" className="absolute top-[17%] left-0 w-full h-px pointer-events-none opacity-0" />
      <div id="offboarding" className="absolute top-[37%] left-0 w-full h-px pointer-events-none opacity-0" />
      <div id="assistance" className="absolute top-[57%] left-0 w-full h-px pointer-events-none opacity-0" />
      ...
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col items-center justify-center">
        ...
        >
          <h1 className={`${spaceGrotesk.className} text-6xl md:text-8xl lg:text-9xl font-bold text-[#0E1B2E] tracking-tighter`}>
            Smarix<br/>Capabilities
          </h1>
        </motion.div>
        ...
        >
          <div className="w-full flex justify-between items-center py-8 border-b border-slate-100/50">
             <div className="flex items-center gap-2">
                <span className={`${spaceGrotesk.className} text-xl font-bold text-[#0E1B2E]`}>Smarix</span>
                <span className="text-slate-300">/</span>
                <span className={`${jetbrainsMono.className} text-sm font-medium text-slate-500`}>Capabilities</span>
             </div>
                ...
                {features.map((_, i) => (
                  <motion.div 
                    ...
                    animate={{ 
                      ...
                      backgroundColor: activeIndex === i ? features[activeIndex].color : '#E2E8F0'
                    }}
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
               ...
               <AnimatePresence mode="wait">
                  ...
                  >
                     <div className="mb-8">
                        ...
                        >
                            {React.createElement(features[activeIndex].icon, { color: features[activeIndex].color, size: 32 })}
                        </motion.div>
                        
                        <h2 className={`${spaceGrotesk.className} text-5xl lg:text-6xl font-bold leading-[1.05] tracking-tight mb-6 text-[#0E1B2E]`}>
                            {features[activeIndex].title}
                        </h2>
                        
                        <p className={`${spaceGrotesk.className} text-xl leading-relaxed text-slate-500`}>
                        ...
                        {features[activeIndex].points.map((point, idx) => (
                           ...
                           >
                              <div className="mt-1 w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center shrink-0">
                                  <point.icon size={16} style={{ color: features[activeIndex].color }} />
                              </div>
                              <div>
                                  <h4 className={`${spaceGrotesk.className} text-base font-bold text-[#0E1B2E] mb-1`}>
                                    {point.title}
                                  </h4>
            ...
            <div className="lg:col-span-7 h-full max-h-[600px] flex items-center">
                <div className="relative w-full aspect-[16/10] rounded-[2rem] bg-white p-2 shadow-2xl shadow-slate-200/50 border border-slate-100 ring-4 ring-slate-50">
                    <AnimatePresence mode="wait">
                       ...
                       >
                          <div className="absolute top-0 left-0 right-0 h-10 bg-white/90 backdrop-blur z-20 border-b border-slate-100 flex items-center px-4 justify-between">
                             ...
                             <div className={`${jetbrainsMono.className} text-[10px] text-slate-400`}>
                                smarix-engine://{features[activeIndex].tag.toLowerCase()}
                             </div>
                          </div>
                    ...
                    >
                       <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: features[activeIndex].color }} />
                       <span className={`${jetbrainsMono.className} text-xs font-bold text-[#0E1B2E]`}>
                         LIVE PREVIEW
                       </span>
                    </motion.div>
                </div>
